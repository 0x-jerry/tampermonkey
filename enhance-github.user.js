...
(function() {
	...
	* @returns {Promise<T>}
	*/
	async function when(checker, timeout = 1e4) {
	...
	*/
	...
	const INFO_ROWS = [{
		...
		icon: sendIcon,
		id: FIRST_COMMIT_DATE_ID,
		title: "First Commit Datetime"
	}];
	run(async () => {
	...
	async function enhance(ctx) {
		...
		try {
			...
			await Promise.all([fillInfo(REPO_SIZE_ID, () => fetchRepoSize(repoInfo)), fillInfo(FIRST_COMMIT_DATE_ID, () => fetchFirstCommitDate(repoInfo))]);
		} catch (err) {
			console.error(err);
		} finally {
			ctx.enhancing = false;
	...
	function createInfoRows() {
		...
		for (const { icon, id, title } of INFO_ROWS) frag.append(html`
	...
	async function fillInfo(id, fetcher) {
	...
	async function fetchRepoSize(repoInfo) {
		const { data } = await fetchGitHub(`https://api.github.com/repos/${repoInfo.owner}/${repoInfo.repo}`);
		const sizeKB = data.size;
		return sizeKB > 1024 ? `${(sizeKB / 1024).toFixed(1)} MB` : `${sizeKB} KB`;
	}
	async function fetchFirstCommitDate(repoInfo) {
		const { headers } = await fetchGitHub(`https://api.github.com/repos/${repoInfo.owner}/${repoInfo.repo}/commits?per_page=1`);
		const lastPageUrl = getLastPageUrl(headers);
		if (!lastPageUrl) return "Unavailable";
		const { data } = await fetchGitHub(lastPageUrl);
		const date = data.at(0)?.commit.committer.date;
		return date ? (0, import_dayjs_min.default)(date).format("YYYY-MM-DD HH:mm:ss") : "Unavailable";
	}
	function getLastPageUrl(headers) {
		const link = headers.get("link");
		if (!link) return;
		for (const part of link.split(",")) {
			const [url, rel] = part.split(";");
			if (rel?.includes("rel=\"last\"")) return url.trim().slice(1, -1);
		}
	}
	//#endregion
})();
